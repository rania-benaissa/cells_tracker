    ...
    for i in pos_ids:
        ...
        if config.USE_MINI_MASK:
            ...
            placeholder[gt_y1:gt_y2, gt_x1:gt_x2] = \
                np.round(utils.resize(class_mask.astype(
                    np.uint8), (gt_h, gt_w))).astype(bool)
        ...
        while b < self.batch_size:
            ...
            image, image_meta, gt_class_ids, gt_boxes, gt_masks = \
                load_image_gt(self.dataset, self.config, image_id,
                              augmentation=self.augmentation, use_mini_mask=True)
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if config.GPU_COUNT > 1:
            from mrcnn1.parallel_model import ParallelModel
